...
const App = () => {
    ...
    const loadIn = () => {                                             // külön fájlban eltárolt szám betöltése a kijelzőbe
        ...
        axios.get("/loadin").then((response) => {  
                    if(result !== "Error"){
                        if(result.concat(response.data.savedNumber.toString()).length <= 15){
                            setResult(result.concat(response.data.savedNumber.toString()));
                        }
                    }
                    else{
    ...
    return (        
        <div className="container">
            <form>
                <input id="screen" type="text" defaultValue={result} />
            </form>
